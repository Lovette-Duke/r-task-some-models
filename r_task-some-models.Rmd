...
Importing csv file
```{r}
setwd("/Code_data_file") #setting working directory as necessary
car_sales <- read_csv("Data/Car_sale_ads.csv")
```
Explore dataset
```{r}
glimpse(car_sales)
...
Building the model
```{r}
dtree_m <- tree(Condition ~ Fuel_type + Drive + Transmission + Type + Colour, car_sales)
```
...
h_fit <- hclust(h_dist, method = "ward.D")
